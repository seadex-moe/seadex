...
function parse$1 (input, loose) {
  ...
  let c; let o; let tmp; let ext; const keys = []; let pattern = ''; const arr = input.split('/')
  arr[0] || arr.shift()
...
function parse (req) {
  ...
  let pathname = raw; let search = ''; let query
...
class Polka extends Trouter {
  ...
  handler (req, res, next) {
    const info = this.parse(req); const path = info.pathname
    ...
    let i = 0; const arr = obj.handlers.concat(this.onNoMatch); const len = arr.length
...
if (socket_activation) {
  ...
  server.listen({ path, host, port }, () => {
    console.log(`Listening on ${path || host + ':' + port}`)
  })
}
